...
import { Canvas, Group, Rect, RoundedRect, Skia, Path as SkiaPath } from "@shopify/react-native-skia";
...
export const AvatarSunset = React.memo((props: AvatarSunsetProps) => {
    ...
    const layerPaths = React.useMemo(() => {
        return layers.map((layer) => {
            ...
            path.moveTo(left, size);
            path.quadTo(left, top, layer.cx, top);
            path.quadTo(right, top, right, size);
            path.close();

            return { path, color: layer.color };
        });
    }, [layers, size]);
    ...
    return (
        <Canvas style={{ width: size, height: size }}>
            <Group clip={clipPath}>
                ...
                {layerPaths.map((lp, i) => (
                    <SkiaPath key={i} path={lp.path} color={lp.color} />
                ))}
            </Group>
        </Canvas>
    );
