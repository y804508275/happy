...
export const zhHant: TranslationStructure = {
    ...
    session: {
        ...
        switchAgentConfirm: ({ agent }: { agent: string }) => `確認要切換到 ${agent} 嗎？`,
        switchingAgent: '正在切換...',
    ...
    knowledgeBase: {
        ...
        alwaysActive: '始終生效',
        onDemand: '按需引用',
        toggleToOnDemand: '切換為按需引用',
        toggleToOnDemandMessage: ({ name }: { name: string }) =>
            `「${name}」將不再自動注入。你仍可在傳送訊息時手動選擇引用。`,
    ...
    mdReference: {
        ...
        globalSection: '全域',
        projectSection: '專案',
        emptyDescription: '在知識庫中建立規則，然後在訊息中引用它們。',
        newButton: '新增',
        createTitle: '新增規則',
        editTitle: '編輯規則',
        nameLabel: '標題',
        namePlaceholder: '規則標題',
        descriptionLabel: '描述',
        descriptionPlaceholder: '簡短描述（選填）',
        contentLabel: '內容',
        contentPlaceholder: '規則內容（Markdown 格式）...',
        scopeLabel: '範圍',
        scopeGlobal: '全域',
        scopeProject: '專案',
        save: '儲存',
        ...
        instructionPlaceholder: '這條規則要做什麼？（如「按此模板產生代碼」）',
    },
