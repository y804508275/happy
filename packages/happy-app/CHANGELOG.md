...
## Version 36 (2026.03.12.4) - 2026-03-12

修复新用户无法看到共享机器的问题。

- 修复服务端对非拥有者机器返回错误加密 key 的 bug
- 客户端增加共享机器 key 解码的错误处理，避免崩溃
- 跳过尚未上传 sharedKey 的共享机器，避免误显示

## Version 35 (2026.03.12.3) - 2026-03-12

修复新用户创建会话体验，优化空状态显示。

- 修复新用户无法自动选择共享机器的问题，共享机器现自动选中
- 空状态保持与正常会话列表一致的布局，显示"暂无对话"提示
- 移除独立的空状态页面，统一使用 SessionsList 组件
- 修复退出登录后重新登录仍提示选择设备的问题

## Version 34 (2026.03.12.2) - 2026-03-12

优化新用户空状态页面，添加创建新对话按钮。

- 新用户登录后显示"暂无对话"空状态和"开始新对话"按钮
- 移除手机端空状态的 CLI 安装引导，改为简洁的创建对话入口
- 平板端空状态始终显示"开始新对话"按钮，不再依赖机器在线状态
- 更新所有语言的翻译文件

## Version 33 (2026.03.12.1) - 2026-03-12

所有机器默认共享，部署即用，无需手动配置。

- 所有机器默认标记为共享，新用户登录即可使用
- 移除 HAPPY_MACHINE_SHARED 环境变量要求，daemon 启动自动共享
- 跨用户 RPC 路由不再需要 shared 标记
- 数据库迁移自动将现有机器更新为共享状态

## Version 32 (2026.03.11.7) - 2026-03-11

多用户会话隔离功能，不同飞书用户登录后拥有独立的会话空间。

- 新增共享机器功能，daemon 可通过 HAPPY_MACHINE_SHARED=true 标记机器为共享
- 不同用户只能看到自己的会话，历史会话与原始用户关联
- 共享机器上创建的会话自动归属于创建者账户
- 支持跨用户 RPC 路由，允许任意用户使用共享机器的 daemon
- CLI 支持通过环境变量 HAPPY_AUTH_TOKEN/HAPPY_AUTH_SECRET 委托认证
- 机器选择器中共享机器显示"共享"标识
...
## Version 28 (2026.03.11.3) - 2026-03-11
...
- 登录页移除"使用移动应用登录"按钮
- 登录页移除"链接或恢复账户"按钮  
- 所有平台（Web/iOS/Android）统一使用飞书 OAuth 登录
